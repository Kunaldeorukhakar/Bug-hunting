...
subfinder -d $target -t 100 -v;python3 ~/Documents/tools/github-subdomains.py -t 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 -d $target | anew  allsub.txt |tee -a newsubdomain_$(date +"%d;%m;%Y_%H;%M;%S").txt | notify -pc ~/Documents/BugBounty/$pc | tee -a allsub.txt
...
trufflehog github --org=$org --results=verified --no-update | anew gitsecrets.txt |tee -a newgitsecrets_$(date +"%d;%m;%Y_%H;%M;%S").txt | notify -pc ~/Documents/BugBounty/$pc | tee -a gitsecrets.txt

echo "[*] Checking for Subdomain takeover..."
subzy run --targets allsub.txt | tee -a newSubdomaintakeover_$(date +"%d;%m;%Y_%H;%M;%S").txt
